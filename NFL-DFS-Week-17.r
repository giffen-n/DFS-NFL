...
setwd("~/DFS/DFS-NFL")
MASTER <- read.csv("MASTER_7-17.csv")
WK17 <- read.csv("TEST_17.csv")
...
# Decision Tree to Predict DraftKings Points
fit <- cforest(DKPts ~ Pos + Last.1 + HI + HI5 + Last.2 + Last.3 + Last.4 + Last.5 + Last.6 + 
             Spread + Total + FPPG + PosDVOA, data=MASTER, 
...
write.csv(submit, file = "Updated-Pred.csv", row.names = FALSE)
...
write.csv(submit, file = "Updated-MASTER-Pred.csv", row.names = FALSE)